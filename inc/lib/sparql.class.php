...
abstract class sparql {
    ...
    protected static function a_query($endpoint_url, $endpoint_id, $query, $cache) {
        ...
        $cache = max(300, $cache);
        $res = db::query('SELECT `query`, `response` FROM `query` WHERE `query_hash` = 0x'.$query_hash.' AND `endpoint_id` = \''.db::sec($endpoint_id).'\' AND `last_update` >= NOW() - INTERVAL '.$cache.' SECOND');
        if ($res->num_rows == 1) {
            $o = $res->fetch_object();
            if ($o->query !== $query) {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_USERAGENT, 'Dicare/2.1 2019-04-08 tools');
